 ...
 */
export function buildProspectFormSchema(i18n: I18n) {
  return z
    .object({
      /**
      ...
      hotelName: z
        .string()
        .trim()
        .min(1, i18n._(msg`Escribe el nombre del hotel`)),
      zoneId: z.string().min(1, i18n._(msg`Elige la zona`)),
      timeZone: z.string().min(1, i18n._(msg`Elige la zona horaria`)),
      ...
      location: z
        ...
        .refine((value) => Boolean(value), { message: i18n._(msg`Marca la ubicación en el mapa`) }),
      ...
      geofenceMeters: z
        .number()
        .int(i18n._(msg`La geocerca se mide en metros enteros`))
        .min(GEOFENCE_MIN_M, i18n._(msg`El radio mínimo es de ${String(GEOFENCE_MIN_M)} m`))
        .max(GEOFENCE_MAX_M, i18n._(msg`El radio máximo es de ${String(GEOFENCE_MAX_M)} m`)),
      ...
      contactEmail: z.union([
        z.literal(''),
        z.email(i18n._(msg`Escribe un correo válido, como marta@hotel.mx`)),
      ]),
      isPrimaryContact: z.boolean(),

      ownerUserId: z.string().min(1, i18n._(msg`Elige al dueño del prospecto`)),
    ...
    .superRefine((values, ctx) => {
      if (values.intent === 'CREATE' && values.contactFullName === '') {
        ctx.addIssue({
          ...
          message: i18n._(msg`El ciclo abre con un contacto: escribe su nombre`),
        })
      }
      ...
      if (values.hotelSource === 'EXISTING' && values.existingHotelId === '') {
        ctx.addIssue({
          ...
          message: i18n._(msg`Elige el hotel ya registrado`),
        })
      }
    })
}

export type ProspectFormValues = z.infer<ReturnType<typeof buildProspectFormSchema>>
