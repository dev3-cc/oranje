/*
 * FoldText de reactbits (https://reactbits.dev/text-animations/fold-text),
 * copiado de su fuente oficial (react-bits, variante TS+Tailwind) sobre GSAP.
 * Ver los defaults adaptados en la firma; el resto no se toca para poder
 * refrescarlo contra la fuente.
 */
...
export const FoldText = ({
  text = 'Design unfolds',
  /* Defaults adaptados a Oranje: los títulos de sección heredan su propia
     tipografía y se pliegan por PALABRA — el resto es el componente verbatim. */
  splitBy = 'word',
  hinge = 'top',
  duration = 0.65,
  stagger = 0.045,
  ...
  useEffect(() => {
    ...
    if (trigger === 'hover') {
      ...
      hoverHandler = () => {
        /* El Timeline de gsap es thenable: se descarta explícito (lint). */
        void play(false)
      }
